...
public class MazeSearchAlgo {
  ...
  public boolean occupy_next_element() {
    ...
    while (!maze_expansion_list.isEmpty()) {
      ...
      if (curr_door_section == null || !curr_door_section.is_occupied) {
  ...
  private boolean expand_to_room_doors(MazeListElement p_list_element) {
    ...
    // Guard against invalid layer index to prevent ArrayIndexOutOfBoundsException
    if (layer_no < 0 || layer_no >= ctrl.layer_active.length) {
      FRLogger.warn("MazeSearchAlgo.expand_to_room_doors: invalid layer_no=" + layer_no);
      return true;
    }
    boolean layer_active = ctrl.layer_active[layer_no];
    if (!layer_active) {
      if (layer_no < autoroute_engine.board.layer_structure.arr.length
          && autoroute_engine.board.layer_structure.arr[layer_no] != null
          && autoroute_engine.board.layer_structure.arr[layer_no].is_signal) {
        return true;
      }
    }

    if (layer_no < 0 || layer_no >= ctrl.compensated_trace_half_width.length) {
      return true;
    }
    ...
    if (p_list_element.next_room instanceof ObstacleExpansionRoom room) {
      ...
      if (next_room_shape == null) {
        FRLogger.warn("MazeSearchAlgo.expand_to_room_doors: next_room_shape is null");
        next_room_is_thick = false;
      } else if (next_room_shape.min_width() < 2 * half_width) {
