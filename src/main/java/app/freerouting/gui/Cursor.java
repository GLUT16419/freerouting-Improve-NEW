...
public abstract class Cursor {
  ...
  protected static void init_graphics(Graphics2D p_graphics) {
    ...
    // Use XOR mode to guarantee cursor visibility against ANY background color.
    // XOR with white means:
    //   black background  → draws white  (inverted)
    //   white element     → draws black  (inverted)
    //   any color element → draws complement (always visible)
    p_graphics.setXORMode(Color.WHITE);
