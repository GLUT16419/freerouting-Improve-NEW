...
public class UndoableObjects implements Serializable {
  ...
  public void save_for_undo(UndoableObjects.Storable p_object) {
    ...
    if (curr_node == null) {
      // This can happen on deep-copied boards where the undo stack level differs
      // from the original board's state after serialization/deserialization.
      // Deep copies are temporary, so a failed undo save is harmless.
      FRLogger.debug("UndoableObjects.save_for_undo: object node not found");
      return;
    }
