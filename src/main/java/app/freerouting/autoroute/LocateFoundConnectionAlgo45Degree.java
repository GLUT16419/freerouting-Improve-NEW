...
public class LocateFoundConnectionAlgo45Degree extends LocateFoundConnectionAlgo {
  ...
  protected Collection<FloatPoint> calculate_next_trace_corners() {
    ...
    BacktrackElement curr_from_info = null;
    if (this.current_to_door_index > 0 && this.current_to_door_index <= this.backtrack_array.length) {
      curr_from_info = this.backtrack_array[this.current_to_door_index - 1];
    }

    if (curr_from_info == null || curr_from_info.next_room == null) {
