...
public class AutorouterAndRouteOptimizerThread extends InteractiveActionThread {
  ...
  protected AutorouterAndRouteOptimizerThread(GuiBoardManager p_board_handling, RoutingJob routingJob) {
    ...
    if (app.freerouting.settings.RouterSettings.ALGORITHM_UTPR.equals(algorithm)) {
      ...
      if (!app.freerouting.settings.RouterSettings.ALGORITHM_CURRENT.equals(algorithm)
          && !app.freerouting.settings.RouterSettings.ALGORITHM_HYBRID.equals(algorithm)) {
  ...
  protected void thread_action() {
    ...
    try {
      ...
      // ── Benchmark mode: run twice (HY/optimized vs baseline) and compare ──
      if (routingJob.routerSettings.benchmarkMode && !this.is_stop_auto_router_requested()) {
        runBenchmark();
        routingJob.routerSettings.benchmarkMode = false; // Reset so normal runs don't re-trigger
      }

      // Let's run the autorouter (normal single run)
      if (!routingJob.routerSettings.benchmarkMode
          && routingJob.routerSettings.getRunRouter() && !this.is_stop_auto_router_requested()) {
  ...
  /**
   * Runs the autorouter twice for benchmark comparison:
   * <ul>
   *   <li><b>Test A (HY/Optimized):</b> Current settings with fanout + all optimizations enabled</li>
   *   <li><b>Test B (Baseline):</b> Same board, fanout disabled (previous behavior)</li>
   * </ul>
   * Results are logged to the routing job output.
   */
  private void runBenchmark() {
    // IMPORTANT: Deep copy the original board BEFORE Test A runs.
    // routingJob.board gets modified by Test A (BatchAutorouter updates the board
    // in-place and may replace the reference via job.board = this.board).
    // Without a deep copy here, Test B would start from the already-routed state.
    RoutingBoard originalBoard = routingJob.board.deepCopy();
    if (originalBoard == null) {
      routingJob.logError("Cannot run benchmark: no board loaded.", new Exception());
      return;
    }

    routingJob.logInfo("═══════════════════════════════════════════════════");
    routingJob.logInfo("  BENCHMARK MODE: Comparing HY vs Baseline");
    routingJob.logInfo("═══════════════════════════════════════════════════");

    // ──── Test A: HY / Optimized (use current batchAutorouter which has GUI listeners) ────
    routingJob.logInfo("─── Test A: HY/Optimized algorithm ───");
    long testAStart = System.currentTimeMillis();
    if (batchAutorouter instanceof BatchAutorouter) {
      ((BatchAutorouter) batchAutorouter).runBatchLoop();
    } else if (batchAutorouter instanceof BatchAutorouterV19) {
      ((BatchAutorouterV19) batchAutorouter).runBatchLoop();
    }
    long testADuration = System.currentTimeMillis() - testAStart;
    BoardStatistics statsA = new BoardStatistics(routingJob.board);
    float scoreA = statsA.getNormalizedScore(routingJob.routerSettings.scoring);
    routingJob.logInfo("  Test A completed in " + (testADuration / 1000.0) + "s"
        + " — score: " + FRLogger.formatScore(scoreA,
            statsA.connections.incompleteCount, statsA.clearanceViolations.totalCount));

    // ──── Restore board for Test B ────
    RoutingBoard boardForTestB = originalBoard.deepCopy();
    routingJob.board = boardForTestB;
    boardManager.replaceRoutingBoard(boardForTestB);

    // ──── Test B: Baseline (fanout disabled, represents previous behavior) ────
    routingJob.logInfo("─── Test B: Baseline (fanout disabled) ───");
    boolean fanoutWasEnabled = routingJob.routerSettings.isFanoutEnabled();
    routingJob.routerSettings.setFanoutEnabled(false);
    // Also disable optimizer for baseline to isolate routing performance
    boolean optimizerWasEnabled = routingJob.routerSettings.optimizer.enabled;
    routingJob.routerSettings.optimizer.enabled = false;

    long testBStart = System.currentTimeMillis();
    BatchAutorouter autorouterB = new BatchAutorouter(routingJob);
    autorouterB.runBatchLoop();
    long testBDuration = System.currentTimeMillis() - testBStart;

    // Restore settings
    routingJob.routerSettings.setFanoutEnabled(fanoutWasEnabled);
    routingJob.routerSettings.optimizer.enabled = optimizerWasEnabled;

    BoardStatistics statsB = new BoardStatistics(routingJob.board);
    float scoreB = statsB.getNormalizedScore(routingJob.routerSettings.scoring);

    // ──── Comparison Report ────
    routingJob.logInfo("═══════════════════════════════════════════════════");
    routingJob.logInfo("  BENCHMARK RESULTS");
    routingJob.logInfo("═══════════════════════════════════════════════════");
    routingJob.logInfo(String.format(
        "  %-25s %15s %15s",
        "Metric", "Test A (HY)", "Test B (Base)"));
    routingJob.logInfo(String.format(
        "  %-25s %14.1fs %14.1fs",
        "Routing time", testADuration / 1000.0, testBDuration / 1000.0));
    routingJob.logInfo(String.format(
        "  %-25s %14.1f  %14.1f",
        "Score", (double) scoreA, (double) scoreB));
    routingJob.logInfo(String.format(
        "  %-25s %14d  %14d",
        "Incompletes", statsA.connections.incompleteCount, statsB.connections.incompleteCount));
    routingJob.logInfo(String.format(
        "  %-25s %14d  %14d",
        "Violations", statsA.clearanceViolations.totalCount, statsB.clearanceViolations.totalCount));
    routingJob.logInfo("═══════════════════════════════════════════════════");

    // Keep the better result
    if (scoreB > scoreA) {
      routingJob.logInfo("Test B (baseline) produced a better score. Keeping Test B result.");
    } else {
      routingJob.logInfo("Test A (HY/optimized) produced a better or equal score. Keeping Test A result.");
      routingJob.board = originalBoard;
      boardManager.replaceRoutingBoard(originalBoard);
    }
  }
}