...
public class ScreenMessages {
  ...
  /** Label for stage elapsed time (fanout/autoroute/optimizer duration). */
  private final JLabel stageTimeLabel;
  ...
  public ScreenMessages(JLabel errorLabel, JLabel warningLabel, JLabel p_status_field, JLabel p_add_field,
    ...
    stageTimeLabel = p_stageTimeLabel;
  ...
  /** Updates the stage elapsed time display (e.g. "Fanout 3.2s" / "Route 12.5s"). */
  public void set_stage_time(String phase, long elapsedMillis) {
    if (stageTimeLabel == null) return;
    String phaseAbbr = phase != null ? phase.substring(0, 1).toUpperCase() + phase.substring(1) : "";
    double secs = elapsedMillis / 1000.0;
    stageTimeLabel.setText(phaseAbbr + " " + String.format("%.1fs", secs));
  }

  /** Clears the stage time display. */
  public void clear_stage_time() {
    if (stageTimeLabel != null) {
  ...
  public void set_batch_autoroute_info(RouterCounters routerCounters) {
    ...
    if ("fanout".equals(routerCounters.phase)) {
      ...
      if (routerCounters.stageElapsedMillis != null) {
        set_stage_time("fanout", routerCounters.stageElapsedMillis);
      }
      return;
    }
    ...
    if (routerCounters.stageElapsedMillis != null) {
      set_stage_time("route", routerCounters.stageElapsedMillis);
    }
  }
