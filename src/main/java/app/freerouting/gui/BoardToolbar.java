...
class BoardToolbar extends JPanel {
  ...
  BoardToolbar(BoardFrame p_board_frame, boolean p_disable_select_mode) {
    ...
    // Add "Settings" button to the toolbar (opens Layer Assignment window)
    settings_button = new JButton();
    tm.setText(settings_button, "settings_button");
    settings_button.addActionListener(_ -> {
      if (board_frame.layer_assignment_window == null
          || !board_frame.layer_assignment_window.isDisplayable()) {
        board_frame.layer_assignment_window = new WindowLayerAssignment(board_frame);
      }
      board_frame.layer_assignment_window.setVisible(true);
    });
    ...
    // Add "Benchmark" button to the toolbar — runs HY vs baseline comparison
    JButton benchmark_button = new JButton();
    tm.setText(benchmark_button, "benchmark_button");
    benchmark_button.setToolTipText("对比模式：HY算法 vs 基准算法（运行两次后比较结果）");
    Font benchFont = new Font(currentFont.getFontName(), Font.PLAIN, currentFont.getSize());
    benchmark_button.setFont(benchFont);
    benchmark_button.setBorder(BorderFactory.createBevelBorder(BevelBorder.RAISED));
    benchmark_button.setBorder(BorderFactory.createCompoundBorder(benchmark_button.getBorder(),
        BorderFactory.createEmptyBorder(2, 5, 2, 5)));
    benchmark_button.setCursor(new java.awt.Cursor(java.awt.Cursor.HAND_CURSOR));
    benchmark_button.addActionListener(_ -> {
      var routingJobs = RoutingJobScheduler
          .getInstance()
          .listJobs(SessionManager
              .getInstance()
      ...
      // Enable benchmark mode: the AutorouterAndRouteOptimizerThread will run twice
      guiRoutingJob.routerSettings.benchmarkMode = true;
      InteractiveActionThread thread = board_frame.board_panel.board_handling
          .start_autorouter_and_route_optimizer(guiRoutingJob);
      if ((thread != null) && (board_frame.board_panel.board_handling.autorouter_listener != null)) {
        thread.addListener(board_frame.board_panel.board_handling.autorouter_listener);
    ...
    benchmark_button.addActionListener(
        _ -> FRAnalytics.buttonClicked("toolbar_benchmark_button", "benchmark"));
    middle_toolbar.add(benchmark_button);
    ...
    // Set explicit foreground color for all toolbar buttons to ensure contrast
    // against the dark toolbar background (new Color(30, 30, 35)).
    Color buttonFg = new Color(100, 150, 255);
    for (Component comp : middle_toolbar.getComponents()) {
      if (comp instanceof JButton btn) {
        btn.setForeground(buttonFg);
      }
    }
    // Also set foreground for debug buttons (added later, but ensure coverage)
    if (vars_play_button != null) vars_play_button.setForeground(buttonFg);
    if (vars_pause_button != null) vars_pause_button.setForeground(buttonFg);
    if (vars_next_button != null) vars_next_button.setForeground(buttonFg);
    if (vars_previous_button != null) vars_previous_button.setForeground(buttonFg);

    // Set SegmentedButtons background to match the dark theme consistently
    modeSelectionPanel.setBackground(new Color(45, 45, 50));
    unitSelectionPanel.setBackground(new Color(45, 45, 50));
  ...
  /**
   * Returns the autoroute button, so it can be triggered programmatically.
   */
  public JButton getAutorouteButton() {
    return toolbar_autoroute_button;
  }
