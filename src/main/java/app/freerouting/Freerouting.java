...
public class Freerouting {
  ...
  private static boolean InitializeDRC(GlobalSettings globalSettings) {
    ...
    // Start a new Freerouting session
    String drcSafeUserId = globalSettings.userProfileSettings.userId;
    if (drcSafeUserId == null || drcSafeUserId.isEmpty()) {
      drcSafeUserId = "00000000-0000-0000-0000-000000000000";
    }
    var drcSession = SessionManager
        .getInstance()
        .createSession(UUID.fromString(drcSafeUserId),
            "Freerouting/" + globalSettings.version);
  ...
  void main(String[] args) {
    ...
    // check for new version (disabled)
    ...
    // Initialize the GUI
    if (globalSettings.guiSettings.isEnabled) {
      // Force Simplified Chinese locale for full Chinese UI support
      globalSettings.currentLocale = Locale.SIMPLIFIED_CHINESE;
