...
public class RouterSettings implements Serializable, Cloneable {
  ...
  /** When true, runs the router twice (HY/optimized vs baseline) and logs comparison results. */
  @SerializedName("benchmark_mode")
  public boolean benchmarkMode = false;
  /**
   * Via type selection: "th" = through-hole (通孔), "bv" = blind/buried (盲埋孔).
   * Default is "th" (through-hole).
   */
  @SerializedName("via_type")
  public String viaType;
  /**
   * The set of net numbers that the user has selected for routing.
   * null means "route all nets" (default behaviour).
   */
  @SerializedName("nets_to_route")
  ...
  public RouterSettings clone() {
    ...
    result.viaType = this.viaType;
    result.automatic_neckdown = this.automatic_neckdown;
    result.maxThreads = this.maxThreads;
    if (this.netsToRoute != null) {
      result.netsToRoute = new java.util.HashSet<>(this.netsToRoute);
    }
    ...
    result.hybrid = this.hybrid != null ? this.hybrid.clone() : new HybridRouterSettings();
