...
public abstract class StoppableThread extends Thread implements Stoppable {
  ...
  /**
   * Resets the auto-router stop request, allowing subsequent routing stages
   * (e.g. last-mile router, P4 constraints) to proceed.
   * Only resets AUTO_ROUTER_ONLY stops — a user-requested ALL stop is preserved.
   *
   * @return true if the stop was reset, false if a user stop was in effect
   */
  public synchronized boolean resetStopRequest() {
